...
//adding new employee
 public function Addemp(Request $request) // Inject Request to get the authenticated user
    {
        $admin = $request->user();
        $departmentId = $admin->department_id;

        // 1. Fetch all Roles (Roles are usually global)
        $roles = Role::all();

        // 2. Fetch only Services belonging to the admin's department
        $services = Service::where('department_id', $departmentId)->get();



        //return view('employees.create', compact('roles', 'services'));
    }
    ...
    public function Saveemp(Request $request)
    {
        ...
        // 2. Validate the incoming data (UPDATED VALIDATION)
        $request->validate([
            'employee_id' => 'required|string|unique:employees,employee_id|max:255', // NEW: Required and unique
            'full_name' => 'required|string|max:255',
            'email' => 'required|email|unique:employees,email|max:255',
            'phone' => 'nullable|string|max:255',
            'password' => 'required|string|min:8',

            // Validate foreign key for Role exists
            'role_id' => 'required|exists:roles,role_id',

            // service_id is now checked against the employee's department implicitly
            // We perform an additional validation step below to ensure security and integrity.
        ...
        // 2b. Security Check: Ensure the selected service_id belongs to the admin's department
        if ($request->filled('service_id')) {
            $service = Service::find($request->service_id);
            if (!$service || $service->department_id !== $departmentId) {
                return back()->withInput()->withErrors(['service_id' => 'The selected service does not belong to your department.']);
            }
        }
        ...
        try {
           Employee::create([
                'employee_id' => $request->employee_id, // NEW: Use the submitted ID
                ...
                // AUTOMATICALLY ASSIGNED: Use the department ID of the logged-in admin
                'department_id' => $departmentId,
            ...
            //return redirect()->route('employees.index')->with('success', 'Employee added successfully and assigned to your department.');
              return response()->json(['success' => true,
        'message' => 'A new employee added successfully!',
