...
function ServicesPage() {
  ...
  const allServices = departmentServices.flatMap(dept => dept.services.map(service => ({
    ...service,
  ...
  const filteredServices = allServices.filter(service => service.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'dark:bg-gray-900' : 'bg-gradient-to-br from-blue-50 to-indigo-100'} py-8 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-6xl mx-auto">
        {/* Header with dark mode toggle */}
        <div className="flex justify-between items-center mb-8">
          <div className="text-center md:text-left">
            <h1 className={`text-4xl font-bold ${darkMode ? 'text-indigo-400' : 'text-indigo-800'} mb-4`}>Our Services</h1>
          ...
          <button
            ...
            className={`p-3 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
              darkMode ? 'bg-indigo-600 text-yellow-200' : 'bg-gray-200 text-gray-700'
            }`}
                ...
                <div className="p-6">
                  <div className="flex items-center mb-4">
                    <div className={`p-3 rounded-lg mr-4 ${
                      ...
                      <svg className={`h-6 w-6 ${darkMode ? 'text-indigo-400' : 'text-indigo-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                currentPage === 1 
                  ...
                  : `${darkMode ? 'bg-indigo-900 text-indigo-200 hover:bg-indigo-800' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}`
              }`}
            >
              Previous
            </button>
